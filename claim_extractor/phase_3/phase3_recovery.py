"""
Phase 3 Recovery: Process prefilter-failed items through the judge.
Outputs to a separate file for independent analysis before merging.
"""

import json
import os
import sys
import argparse
import asyncio
import aiohttp
import time
import logging
import signal
from typing import Dict, List, Optional, Tuple

# ============================================================================
# CONFIG
# ============================================================================

INPUT_PHASE2 = "./claim_extractor/phase_2/output/phase2_qa_pairs.jsonl"
MAIN_OUTPUT = "./claim_extractor/phase_3/output/phase3_validated.jsonl"
OUTPUT_DIR = "./claim_extractor/phase_3/output/recovery"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "phase3_recovered.jsonl")
ERROR_FILE = os.path.join(OUTPUT_DIR, "phase3_recovery_errors.jsonl")
CHECKPOINT_FILE = os.path.join(OUTPUT_DIR, "checkpoint.json")
STATS_FILE = os.path.join(OUTPUT_DIR, "stats.json")

MODEL = "meta-llama/llama-3.1-8b-instruct"
CHECKPOINT_INTERVAL = 500

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)
logger = logging.getLogger(__name__)

shutdown_requested = False

def signal_handler(signum, frame):
    global shutdown_requested
    logger.info("Shutdown requested, finishing current batch...")
    shutdown_requested = True

signal.signal(signal.SIGINT, signal_handler)
signal.signal(signal.SIGTERM, signal_handler)

# ============================================================================
# PROMPTS (same as phase3_judge.py)
# ============================================================================

JUDGE_SYSTEM_PROMPT = """You are evaluating whether a rationale supports an answer to a question about a genetic mutation.

Your task: Determine if the provided RATIONALE logically supports the provided ANSWER to the QUESTION.

IMPORTANT:
- Focus on whether the rationale's evidence supports the answer's conclusion
- Check that Yes/No answers match the rationale's implications
- A rationale that describes the opposite of what the answer claims means NOT SUPPORTED
- The rationale must provide evidence for the specific claim in the answer

Output JSON only:
{
  "supported": true,
  "reasoning": "Brief explanation"
}

or

{
  "supported": false,
  "reasoning": "Brief explanation"
}"""

JUDGE_USER_PROMPT = """MUTATION: {mutation}

QUESTION: {question}

ANSWER: {answer}

RATIONALE: {rationale}

Does this rationale support this answer? Output JSON only."""

# ============================================================================
# ORIGINAL PREFILTER (to identify which items to recover)
# ============================================================================

def would_fail_prefilter(item: Dict) -> Tuple[bool, str]:
    """Check if item would have failed the original prefilter."""
    mutation = item.get("mutation", "").lower()

    if not item.get("phase2_answerable", True):
        return True, "phase2_not_answerable"

    if mutation and mutation not in item.get("phase1_answer", "").lower():
        return True, "mutation_not_in_phase1_answer"

    if mutation and mutation not in item.get("phase2_answer", "").lower():
        return True, "mutation_not_in_phase2_answer"

    if not item.get("phase1_rationale"):
        return True, "missing_phase1_rationale"

    if not item.get("phase2_rationale"):
        return True, "missing_phase2_rationale"

    return False, "would_pass"

# ============================================================================
# MINIMAL VALIDATION (just check we have enough data to judge)
# ============================================================================

def can_be_judged(item: Dict) -> Tuple[bool, str]:
    """Minimal check - just need answers and rationales to exist."""
    if not item.get("phase1_answer"):
        return False, "missing_phase1_answer"
    if not item.get("phase2_answer"):
        return False, "missing_phase2_answer"
    if not item.get("phase1_rationale"):
        return False, "missing_phase1_rationale"
    if not item.get("phase2_rationale"):
        return False, "missing_phase2_rationale"
    return True, "ok"

# ============================================================================
# CLIENT
# ============================================================================

class JudgeClient:
    def __init__(self, api_key: str):
        self.api_key = api_key
        self.base_url = "https://openrouter.ai/api/v1"
        self.semaphore = None

    async def judge(
        self,
        session: aiohttp.ClientSession,
        mutation: str,
        question: str,
        answer: str,
        rationale: str,
    ) -> Tuple[Optional[bool], str, Optional[str]]:
        """Judge if rationale supports answer. Returns (supported, reasoning, error)."""

        headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json",
            "HTTP-Referer": "https://github.com/mutation-qa-generator",
        }

        payload = {
            "model": MODEL,
            "messages": [
                {"role": "system", "content": JUDGE_SYSTEM_PROMPT},
                {"role": "user", "content": JUDGE_USER_PROMPT.format(
                    mutation=mutation, question=question, answer=answer, rationale=rationale
                )},
            ],
            "max_tokens": 300,
            "temperature": 0.1,
        }

        for attempt in range(3):
            try:
                async with self.semaphore:
                    async with session.post(
                        f"{self.base_url}/chat/completions",
                        headers=headers,
                        json=payload,
                        timeout=aiohttp.ClientTimeout(total=60)
                    ) as resp:
                        if resp.status == 200:
                            data = await resp.json()
                            content = data["choices"][0]["message"]["content"].strip()
                            if content.startswith("```"):
                                content = "\n".join(l for l in content.split("\n") if not l.startswith("```"))
                            try:
                                parsed = json.loads(content)
                                return parsed.get("supported"), parsed.get("reasoning", ""), None
                            except:
                                return None, "", f"Parse error: {content[:100]}"
                        elif resp.status == 429:
                            await asyncio.sleep(5 * (attempt + 1))
                        else:
                            text = await resp.text()
                            return None, "", f"HTTP {resp.status}: {text[:100]}"
            except asyncio.TimeoutError:
                await asyncio.sleep(2)
            except Exception as e:
                return None, "", str(e)

        return None, "", "Max retries exceeded"


async def process_item(client: JudgeClient, session: aiohttp.ClientSession, item: Dict) -> Tuple[Optional[Dict], Optional[str]]:
    """Process single item through both judge tests."""

    mutation = item["mutation"]
    question = item["question"]

    # Test 1: R2 -> A1
    r2_a1_supported, r2_a1_reasoning, err1 = await client.judge(
        session, mutation, question,
        item["phase1_answer"],
        item["phase2_rationale"]
    )
    if err1:
        return None, f"R2->A1: {err1}"

    # Test 2: R1 -> A2
    r1_a2_supported, r1_a2_reasoning, err2 = await client.judge(
        session, mutation, question,
        item["phase2_answer"],
        item["phase1_rationale"]
    )
    if err2:
        return None, f"R1->A2: {err2}"

    # Determine grounding
    if r2_a1_supported and r1_a2_supported:
        grounding = "cross-grounded"
    elif r2_a1_supported:
        grounding = "grounded"
    else:
        grounding = "ungrounded"

    result = {
        **item,
        "r2_supports_a1": r2_a1_supported,
        "r2_a1_reasoning": r2_a1_reasoning,
        "r1_supports_a2": r1_a2_supported,
        "r1_a2_reasoning": r1_a2_reasoning,
        "grounding": grounding,
        "recovery_source": "prefilter_bypass",  # Mark as recovered item
    }

    return result, None


def load_checkpoint() -> set:
    """Load processed keys from checkpoint."""
    if os.path.exists(CHECKPOINT_FILE):
        with open(CHECKPOINT_FILE) as f:
            return set(json.load(f).get("processed_keys", []))
    return set()


def save_checkpoint(processed_keys: set, stats: dict):
    """Save checkpoint and stats."""
    with open(CHECKPOINT_FILE, "w") as f:
        json.dump({"processed_keys": list(processed_keys)}, f)
    with open(STATS_FILE, "w") as f:
        json.dump(stats, f, indent=2)


def make_key(item: Dict) -> str:
    return f"{item['pmid']}|{item['mutation']}|{item['question'][:80]}"


def load_already_processed() -> set:
    """Load keys already processed in main Phase 3 output."""
    keys = set()
    if os.path.exists(MAIN_OUTPUT):
        with open(MAIN_OUTPUT) as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
                try:
                    item = json.loads(line)
                    keys.add(make_key(item))
                except json.JSONDecodeError:
                    continue
    return keys


def load_prefilter_failed_items(already_processed: set, recovery_processed: set) -> Tuple[List[Dict], Dict]:
    """Load items that would have failed the original prefilter."""
    items = []
    reason_counts = {}

    with open(INPUT_PHASE2) as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            try:
                item = json.loads(line)
                key = make_key(item)

                # Skip if already processed in main output or recovery
                if key in already_processed or key in recovery_processed:
                    continue

                # Check if it would have failed prefilter
                would_fail, reason = would_fail_prefilter(item)
                if would_fail:
                    # Check if we can still judge it
                    can_judge, judge_reason = can_be_judged(item)
                    if can_judge:
                        item["original_prefilter_reason"] = reason
                        items.append(item)
                        reason_counts[reason] = reason_counts.get(reason, 0) + 1
                    else:
                        # Track items we truly can't process
                        skip_reason = f"skip_{judge_reason}"
                        reason_counts[skip_reason] = reason_counts.get(skip_reason, 0) + 1

            except json.JSONDecodeError:
                continue

    return items, reason_counts


async def main():
    parser = argparse.ArgumentParser(description="Phase 3 Recovery: Process prefilter-failed items")
    parser.add_argument("--workers", type=int, default=50)
    parser.add_argument("--resume", action="store_true")
    parser.add_argument("--limit", type=int, default=None, help="Limit items to process (for testing)")
    parser.add_argument("--dry-run", action="store_true", help="Just show what would be processed")
    args = parser.parse_args()

    api_key = os.environ.get("OPENROUTER_API_KEY")
    if not api_key and not args.dry_run:
        logger.error("OPENROUTER_API_KEY not set")
        sys.exit(1)

    os.makedirs(OUTPUT_DIR, exist_ok=True)

    # Load what's already been processed
    logger.info("Loading already-processed items from main Phase 3 output...")
    already_processed = load_already_processed()
    logger.info(f"Found {len(already_processed):,} items in main output")

    # Load recovery checkpoint
    recovery_processed = load_checkpoint() if args.resume else set()
    logger.info(f"Recovery checkpoint: {len(recovery_processed):,} already processed")

    # Load prefilter-failed items
    logger.info("Loading prefilter-failed items from Phase 2...")
    items, reason_counts = load_prefilter_failed_items(already_processed, recovery_processed)

    logger.info(f"\nPrefilter failure breakdown:")
    for reason, count in sorted(reason_counts.items(), key=lambda x: -x[1]):
        logger.info(f"  {reason}: {count:,}")
    logger.info(f"Total recoverable: {len(items):,}")

    if args.dry_run:
        logger.info("\n--dry-run specified, exiting without processing")
        return

    if args.limit:
        items = items[:args.limit]
        logger.info(f"Limited to {len(items):,} items")

    if not items:
        logger.info("No items to process")
        return

    # Initialize stats
    stats = {
        "cross-grounded": 0,
        "grounded": 0,
        "ungrounded": 0,
        "errors": 0,
        "by_original_reason": {},
    }

    # Load existing stats if resuming
    if args.resume and os.path.exists(STATS_FILE):
        with open(STATS_FILE) as f:
            stats = json.load(f)

    client = JudgeClient(api_key)
    client.semaphore = asyncio.Semaphore(args.workers)

    start_time = time.time()
    total_processed_this_run = 0

    async with aiohttp.ClientSession() as session:
        # Process in batches
        batch_size = args.workers
        for batch_start in range(0, len(items), batch_size):
            if shutdown_requested:
                break

            batch = items[batch_start:batch_start + batch_size]

            tasks = [process_item(client, session, item) for item in batch]
            results = await asyncio.gather(*tasks, return_exceptions=True)

            for item, result in zip(batch, results):
                key = make_key(item)
                recovery_processed.add(key)
                original_reason = item.get("original_prefilter_reason", "unknown")

                if isinstance(result, Exception):
                    stats["errors"] += 1
                    with open(ERROR_FILE, "a") as f:
                        f.write(json.dumps({"key": key, "error": str(result), "original_reason": original_reason}) + "\n")
                elif result[0] is not None:
                    grounding = result[0]["grounding"]
                    stats[grounding] += 1

                    # Track by original prefilter reason
                    if original_reason not in stats["by_original_reason"]:
                        stats["by_original_reason"][original_reason] = {"cross-grounded": 0, "grounded": 0, "ungrounded": 0}
                    stats["by_original_reason"][original_reason][grounding] += 1

                    with open(OUTPUT_FILE, "a") as f:
                        f.write(json.dumps(result[0]) + "\n")
                else:
                    stats["errors"] += 1
                    with open(ERROR_FILE, "a") as f:
                        f.write(json.dumps({"key": key, "error": result[1], "original_reason": original_reason}) + "\n")

            total_processed_this_run += len(batch)

            # Progress
            elapsed = time.time() - start_time
            rate = total_processed_this_run / elapsed if elapsed > 0 else 0
            total_grounded = stats["cross-grounded"] + stats["grounded"]
            total_all = total_grounded + stats["ungrounded"]
            grounded_pct = 100 * total_grounded / total_all if total_all > 0 else 0

            if total_processed_this_run % 500 < batch_size:
                logger.info(
                    f"Processed: {total_processed_this_run:,} / {len(items):,} | "
                    f"Grounded: {total_grounded:,} ({grounded_pct:.1f}%) | "
                    f"Ungrounded: {stats['ungrounded']:,} | "
                    f"Rate: {rate:.1f}/s"
                )

            # Checkpoint
            if total_processed_this_run % CHECKPOINT_INTERVAL < batch_size:
                save_checkpoint(recovery_processed, stats)

    # Final save
    save_checkpoint(recovery_processed, stats)

    elapsed = time.time() - start_time
    total_grounded = stats["cross-grounded"] + stats["grounded"]

    logger.info("=" * 60)
    logger.info("RECOVERY COMPLETE")
    logger.info("=" * 60)
    logger.info(f"Processed this run: {total_processed_this_run:,}")
    logger.info(f"Cross-grounded: {stats['cross-grounded']:,}")
    logger.info(f"Grounded: {stats['grounded']:,}")
    logger.info(f"Ungrounded: {stats['ungrounded']:,}")
    logger.info(f"Errors: {stats['errors']:,}")
    logger.info(f"Time: {elapsed/60:.1f} minutes")

    logger.info("\n--- By Original Prefilter Reason ---")
    for reason, counts in stats.get("by_original_reason", {}).items():
        total = sum(counts.values())
        grounded = counts.get("cross-grounded", 0) + counts.get("grounded", 0)
        pct = 100 * grounded / total if total > 0 else 0
        logger.info(f"  {reason}: {total:,} total, {grounded:,} grounded ({pct:.1f}%)")

    logger.info(f"\nOutput written to: {OUTPUT_FILE}")


if __name__ == "__main__":
    asyncio.run(main())
